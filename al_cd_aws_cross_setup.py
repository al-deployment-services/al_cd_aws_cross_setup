...
def authenticate(user, paswd, yarp):
    ...
    password = paswd
    r = requests.post(
        'https://{0}/aims/v1/authenticate'.format(url), auth=(user, password), verify=False)
...
def post_credentials(token, payload, target_cid):
    ...
    REQUEST = requests.post(API_ENDPOINT, headers={
                            'x-aims-auth-token': token}, verify=False, data=payload)
    print("Create Credentials Status : " +
...
def prep_source_environment(aws_account, cred_id, x_cred_id, environment_name, defender_location):
    ...
    RESULT['source']['config']['aws']['defender_location_id'] = str(
        defender_location)
...
def list_source_environments(token, target_cid):
    ...
    API_ENDPOINT = ALERT_LOGIC_CI_SOURCE + target_cid + "/sources/"
    REQUEST = requests.get(API_ENDPOINT, headers={
                           'x-aims-auth-token': token}, verify=False)

    print("Retrieving Environment info status : " +
          str(REQUEST.status_code), str(REQUEST.reason), file=sys.stderr)
...
def del_source_environment(token, target_env, target_cid):
    ...
    REQUEST = requests.delete(API_ENDPOINT, headers={
                              'x-aims-auth-token': token}, verify=False)
    print("Delete Environment Status : " +
          str(REQUEST.status_code), str(REQUEST.reason))


def del_source_credentials(token, target_cred, target_cid):
    # Delete the specified credentials by credentials ID and CID
    API_ENDPOINT = ALERT_LOGIC_CI_SOURCE + \
        target_cid + "/credentials/" + target_cred
    REQUEST = requests.delete(API_ENDPOINT, headers={
                              'x-aims-auth-token': token}, verify=False)
    print("Delete Credentials Status : " +
          str(REQUEST.status_code), str(REQUEST.reason))
...
def failback(token, cred_id, x_cred_id, target_cid):
    ...
    print("Delete Credentials Status : " +
          str(REQUEST.status_code), str(REQUEST.reason))
    if args.x_arn is not None:
        X_API_ENDPOINT = ALERT_LOGIC_CI_SOURCE + \
            target_cid + "/credentials/" + x_cred_id
        X_REQUEST = requests.delete(X_API_ENDPOINT, headers={
                                    'x-aims-auth-token': token}, verify=False)
        print("Delete X_Credentials Status : " +
              str(X_REQUEST.status_code), str(X_REQUEST.reason))
...
if __name__ == '__main__':
    ...
    add_parser.add_argument(
        "--user", required=True, help="User name / email address for API Authentication")
    add_parser.add_argument("--pswd", required=True,
                            help="Password for API Authentication")
    add_parser.add_argument(
        "--cid", required=True, help="Alert Logic Customer CID as target for this deployment")
    add_parser.add_argument(
        "--aws", required=True, help="Customer AWS Account Number where IAM role is deployed")
    ...
    add_parser.add_argument("--x_arn", required=False,
                            help="Cross Account IAM role arn for centralized cloudtrail")
    add_parser.add_argument("--x_ext", required=False,
                            help="External ID specified in IAM role trust relationship for the centralized account")
    add_parser.add_argument("--x_cred", required=False,
                            help="Centralized account credential name, free form label, not visible in Alert Logic UI")
    add_parser.add_argument(
        "--env", required=True, help="Environment name, will be displayed in Alert Logic UI under Deployment")
    add_parser.add_argument(
        "--dc", required=True, help="Alert Logic Data center assignment, i.e. defender-us-denver, defender-us-ashburn or defender-uk-newport")

    # Parser argumetn for DEL
    del_parser.add_argument(
        "--user", required=True, help="User name / email address for API Authentication")
    del_parser.add_argument("--pswd", required=True,
                            help="Password for API Authentication")
    del_parser.add_argument(
        "--cid", required=True, help="Alert Logic Customer CID where the environment belongs")
    del_parser.add_argument("--envid", required=True,
                            help="Environment ID that you wish to remove")

    # Parser argumetn for LIST
    list_parser.add_argument(
        "--user", required=True, help="User name / email address for API Authentication")
    list_parser.add_argument("--pswd", required=True,
                             help="Password for API Authentication")
    list_parser.add_argument(
        "--cid", required=True, help="Alert Logic Customer CID where the environment belongs")
    ...
    if args.mode == "ADD":
        ...
        CRED_PAYLOAD = prep_credentials(
            TARGET_IAM_ROLE_ARN, TARGET_EXT_ID, TARGET_CRED_NAME)
        CRED_RESULT = post_credentials(TOKEN, str(
            json.dumps(CRED_PAYLOAD, indent=4)), TARGET_CID)
        CRED_ID = str(CRED_RESULT['credential']['id'])

        if args.x_arn is not None:
            X_CRED_PAYLOAD = prep_aux_credentials(
                TARGET_X_IAM_ROLE_ARN, TARGET_X_EXT_ID, TARGET_X_CRED_NAME)
            X_CRED_RESULT = post_credentials(TOKEN, str(
                json.dumps(X_CRED_PAYLOAD, indent=4)), TARGET_CID)
        ...
        if CRED_ID != "n/a":
            ...
            ENV_RESULT = post_source_environment(TOKEN, str(
                json.dumps(ENV_PAYLOAD, indent=4)), TARGET_CID)
            ...
            if ENV_ID != "n/a":
                ...
                EXIT_CODE = 2
                print(
                    "Failed to create environment source, see response code + reason above, starting fallback ..")
                failback(TOKEN, CRED_ID, X_CRED_ID, TARGET_CID)

        else:
            EXIT_CODE = 2
            print(
                "Failed to create credentials, see response code + reason above, stopping ..")
        ...
        SOURCE_RESULT = get_source_environment(
            TOKEN, TARGET_ENV_ID, TARGET_CID)
        ...
        if SOURCE_RESULT["source"]["id"] != "n/a":
            ...
            EXIT_CODE = 2
            print(
                "Failed to find the environment ID, see response code + reason above, stopping ..")
